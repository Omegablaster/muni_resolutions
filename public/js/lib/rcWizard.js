    ...
    return {
      ...
      controller: ['$scope', function ($scope) {
        ...
        var onTabShow = function (activeTab, navigation, currentIndex) {
          ...
          if (currentIndex > 0) {
            wizardElement
              .find('.bottom li:gt(' + (currentIndex - 1) + ')')
              .removeClass("success");
            wizardElement.find('.bottom li:lt(' + currentIndex + ')')
              .addClass("success");
            wizardElement.find('.bottom li a i').remove();
            wizardElement.find('.bottom li:lt(' + currentIndex + ') a').prepend('<i class="glyphicon glyphicon-thumbs-up"></i>');
          } else {
            wizardElement.find('.bottom li').removeClass("success");
            wizardElement.find('.bottom li a i').remove();
          }
